...
def parse_robot(line):
    res = parse('p={x:d},{y:d} v={dx:d},{dy:d}', line)
    return res['x'], res['y'], res['dx'], res['dy']

def sim(x, y, dx, dy, turns):
    m, n = xmax, ymax
    x += dx * turns
    y += dy * turns
    x %= m
...
def is_convex(row):
    try:
        left_robot_index = min([i for i in range(len(row)) if row[i] > 0])
        rigth_robot_index = max([i for i in range(len(row)) if row[i] > 0])
    except ValueError:
        return True
    return not(0 in row[left_robot_index:(rigth_robot_index+1)])
